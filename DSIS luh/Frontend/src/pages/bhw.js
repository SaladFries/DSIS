...
const UserScreen = ({ userType, onLogout }) => {
  ...
  const [InitialSuspectedCases, setSusPatientInfo] = useState([])
  useEffect(()=>{
    fetch('http://localhost:5001/suspectedcasesfrombhw')
    .then(res=>{return res.json()})
    .then(InitialSuspectedCases=>setSusPatientInfo(InitialSuspectedCases))
    .then(err=>console.log(err))
  }, [])

  const [InitialSymptomsInfo, setSymptomsInfo] = useState([])
  useEffect(()=>{
    fetch('http://localhost:5001/symptomsinfo')
    .then(res=>{return res.json()})
    .then(InitialSymptomsInfo=>setSymptomsInfo(InitialSymptomsInfo))
    .then(err=>console.log(err))
  }, [])
  ...
  const specificBHWCases = InitialSuspectedCases.filter (Sus => Sus.AcctID === BHWID);
  const patientMax = Math.max(...InitialSuspectedCases.map(o => o.Patient_ID))
  const [patientCount, setPatientCount] = useState(patientMax);
  const [reportedCases, setReportedCases] = useState(specificBHWCases);
  ...
  const handleRefresh = (e) =>{
    e.preventDefault();
    setReportedCases(specificBHWCases);
    setPatientCount(patientMax);
  }
  ...
  const handleSubmitReport = (e) => {
    ...
    const symptomsList = symptoms
      ...
      /*const patientdata = {
        Patient_ID: PatientCount.count + 1,
        Patient_LastName: patientLName,
        Patient_FirstName: patientFName,
        Patient_MI: patientMI,
        Patient_Suffix: patientSuffix,
        Patient_Bgy: patientBgy,
        Patient_Muni: patientMun,
        Patient_Province: patientProv,
        Patient_Bday: patientBdate,
      }
      const requestpatientOptions = {
        method: "POST",
        headers: { 'Content-type': 'application/json'},  
        body: JSON.stringify(patientdata)
      };
      fetch('http://localhost:5001/suspectedcasesfrombhw/addpatientinfo', requestpatientOptions)
        .then(response => console.log(response.status))
        .catch((err)=>console.log(err));

      const suspatientdata = {
        PotentialSuspect_ID: SuspectedCaseCount.count + 1,
        Notifiable_Disease_ID: 12,
        Patient_ID: patientCount + 1,
        AcctID: BHWID,
        potential_suspect_symptoms: symptomsList,
      }
      const requestOptions = {
        method: "POST",
        headers: { 'Content-type': 'application/json'},  
        body: JSON.stringify(suspatientdata)
      };
      fetch('http://localhost:5001/suspectedcasesfrombhw/addpotentialsuspect', requestOptions)
        .then(response => console.log(response.status))
        .catch((err)=>console.log(err)); */ 
    ...
    setDiseasePredictions([]);
    setPatientCount(newCase.Patient_ID);

    //NEXT AY YUNG NOTIFICATION SA RHU
  };
    ...
    <div className="user-screen">
     ...
     <main className="main-content">
                ...
                <div key={symptom.id} className="symptom-row">
                  <div className="form-group">
                    ...
                    >
                      ...
                      {InitialSymptomsInfo.map((option) => (
                        <option key={option.Symptom_ID} value={option.Symptom_Name}>
                          {option.Symptom_Name}
                        </option>
                      ))}
                    </select>
          ...
          <table>
            ...
            <tbody>
              {reportedCases.map((caseItem) => (
                <tr key={caseItem.Patient_ID}>
                  <td>{caseItem.Patient_ID}</td>
                  <td>{caseItem.Patient_LastName}</td>
                  <td>{caseItem.Patient_FirstName}</td>
                  <td>{caseItem.Patient_MI}</td>
                  <td>{caseItem.Patient_Bgy}</td>
                  <td>{caseItem.Patient_Muni}</td>
                  <td>{caseItem.Patient_Bday}</td>
                  <td>{caseItem.potential_suspect_symptoms}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <footer className="footer">
        ...
        <button className="refresh-button-cases" onClick={handleRefresh}>
            Refresh
          </button>
      </footer>
    </div>
